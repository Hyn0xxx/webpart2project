...
<head>
    ...
    <title>AutoElite - Заказ автомобилей</title>
    ...
    <style>
        ...
        .form-field input, .form-field select, .form-field textarea { width: 100%; padding: 12px 15px; border: 2px solid #e0e0e0; border-radius: 10px; font-size: 1em; }
        .form-field input:focus, .form-field select:focus, .form-field textarea:focus { outline: none; border-color: #9E9E9E; }
        ...
        .btn-send { background: #800020; color: white; border: none; padding: 14px 30px; font-size: 1em; font-weight: 600; border-radius: 10px; cursor: pointer; width: 100%; }
        ...
        .admin-button { position: fixed; bottom: 20px; right: 20px; background: #800020; color: white; padding: 12px 20px; border-radius: 30px; font-weight: 600; box-shadow: 0 5px 15px rgba(0,0,0,0.2); z-index: 1000; display: flex; align-items: center; gap: 8px; cursor: pointer; border: none; }
        .admin-button:hover { background: #9E9E9E; color: #800020; }
        ...
        .modal-window { background: white; width: 90%; max-width: 450px; border-radius: 20px; padding: 30px; position: relative; }
        ...
        @media (max-width: 768px) { .order-body { padding: 20px; } .radio-group { flex-direction: column; gap: 10px; } }
    </style>
</head>
<body>
    <header class="header">
        ...
        <nav class="navbar">
            <div class="container nav-container">
                <div class="logo"><h1>AutoElite</h1><p>Премиальный автосалон с 2010 года</p></div>
                <ul class="nav-menu">
        ...
        <div class="mobile-menu" id="mobileMenu">
            <div class="mobile-menu-header"><h2>AutoElite</h2><button class="close-menu" id="closeMenuBtn"><i class="fas fa-times"></i></button></div>
            <ul class="mobile-nav"><li><a href="#home">Главная</a></li><li><a href="#services">Услуги</a></li><li><a href="#contacts">Контакты</a></li><li><a href="#order-form">Заказать авто</a></li></ul>
        </div>
        <div class="hero"><div class="container"><h2>Эксклюзивные автомобили премиум-класса</h2><p>Подберем идеальный автомобиль по вашим требованиям</p><a href="#order-form" class="btn-hero">Заказать авто</a></div></div>
    </header>
    <main>
                        ...
                        <?php if (!isset($_SESSION['user_id'])): ?>
                            <div class="auth-box">
                                ...
                                <form method="POST" id="loginForm">
                                    <div class="form-field"><label>Логин</label><input type="text" name="login" id="loginInput"></div>
                                    <div class="form-field"><label>Пароль</label><input type="password" name="password" id="passwordInput"></div>
                                    <button type="submit" name="login_submit" class="btn-send">Войти</button>
                                </form>
                            </div>
                            <hr>
                        <?php else: ?>
                            <div class="success-msg">✅ Вы авторизованы как <?= htmlspecialchars($values['full_name']) ?> <a href="?logout=1" class="logout-link">Выйти</a></div>
                        <?php endif; ?>
                        
                        <form method="POST" id="orderForm">
                            <div class="form-field"><label>ФИО *</label><input type="text" name="full_name" id="full_name" value="<?= htmlspecialchars($values['full_name'] ?? '') ?>"><div class="error-msg" id="full_name_error"><?= $errors['full_name'] ?? '' ?></div></div>
                            <div class="form-field"><label>Телефон *</label><input type="tel" name="phone" id="phone" value="<?= htmlspecialchars($values['phone'] ?? '') ?>"><div class="error-msg" id="phone_error"><?= $errors['phone'] ?? '' ?></div></div>
                            <div class="form-field"><label>E-mail *</label><input type="email" name="email" id="email" value="<?= htmlspecialchars($values['email'] ?? '') ?>"><div class="error-msg" id="email_error"><?= $errors['email'] ?? '' ?></div></div>
                            <div class="form-field"><label>Дата рождения *</label><input type="date" name="birth_date" id="birth_date" value="<?= htmlspecialchars($values['birth_date'] ?? '') ?>"><div class="error-msg" id="birth_date_error"><?= $errors['birth_date'] ?? '' ?></div></div>
                            <div class="form-field"><label>Пол *</label><div class="radio-group"><label><input type="radio" name="gender" value="male" <?= (($values['gender'] ?? '') == 'male') ? 'checked' : '' ?>> Мужской</label><label><input type="radio" name="gender" value="female" <?= (($values['gender'] ?? '') == 'female') ? 'checked' : '' ?>> Женский</label><label><input type="radio" name="gender" value="other" <?= (($values['gender'] ?? '') == 'other') ? 'checked' : '' ?>> Другой</label></div><div class="error-msg" id="gender_error"><?= $errors['gender'] ?? '' ?></div></div>
                            <div class="form-field"><label>Интересующие автомобили * (можно выбрать несколько)</label><select name="cars[]" id="cars" multiple><?php foreach ($carsList as $car): ?><option value="<?= $car['id'] ?>" <?= in_array($car['id'], $values['cars'] ?? []) ? 'selected' : '' ?>><?= htmlspecialchars($car['name']) ?> - <?= htmlspecialchars($car['price']) ?></option><?php endforeach; ?></select><div class="error-msg" id="cars_error"><?= $errors['cars'] ?? '' ?></div></div>
                            <div class="form-field"><label>Пожелания к заказу</label><textarea name="wish" id="wish" rows="4" placeholder="Цвет, комплектация, дополнительные опции..."><?= htmlspecialchars($values['wish'] ?? '') ?></textarea></div>
                            <div class="form-field"><label class="checkbox-field"><input type="checkbox" name="contract" value="1" id="contract" <?= !empty($values['contract']) ? 'checked' : '' ?>> Я согласен с условиями обработки данных *</label><div class="error-msg" id="contract_error"><?= $errors['contract'] ?? '' ?></div></div>
                            <button type="submit" class="btn-send" id="formSubmitBtn"><?= isset($_SESSION['user_id']) ? '✏️ Обновить заявку' : '🚗 Отправить заявку' ?></button>
                        </form>
                        <?php if (!isset($_SESSION['user_id'])): ?>
        ...
        <footer class="footer" id="contacts">
            <div class="container">
                <div class="footer-content">
                    <div class="footer-info"><h3>AutoElite</h3><p>Премиальный автосалон с 2010 года</p><p>Москва, ул. Автозаводская, 25</p><p>+7 (495) 123-45-67</p><p>info@autoelite.ru</p></div>
                    <div class="footer-hours"><h4>Часы работы</h4><p>Пн-Пт: 9:00 - 21:00</p><p>Сб: 10:00 - 20:00</p><p>Вс: 10:00 - 18:00</p></div>
                    <div class="footer-social"><h4>Мы в соцсетях</h4><div class="social-icons"><a href="#"><i class="fab fa-vk"></i></a><a href="#"><i class="fab fa-telegram"></i></a><a href="#"><i class="fab fa-instagram"></i></a><a href="#"><i class="fab fa-youtube"></i></a></div></div>
                </div>
                <div class="footer-bottom"><p>&copy; 2024 AutoElite. Все права защищены.</p></div>
            </div>
        </footer>
    </main>
    
    <button class="admin-button" id="adminBtn"><i class="fas fa-shield-alt"></i> Администратору</button>
    <div class="modal-overlay hidden" id="adminModal"><div class="modal-window"><button class="modal-close-btn" id="modalClose"><i class="fas fa-times"></i></button><h2 class="modal-title">🔐 Доступ администратора</h2><form id="adminLoginForm"><div class="form-field"><label>Логин администратора</label><input type="text" id="adminLogin" placeholder="Введите логин" required></div><div class="form-field"><label>Пароль администратора</label><input type="password" id="adminPassword" placeholder="Введите пароль" required></div><button type="submit" class="btn-send">Войти в админ-панель</button><div id="adminError" class="error-msg" style="margin-top: 15px; text-align: center;"></div></form></div></div>
